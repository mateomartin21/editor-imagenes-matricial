...
function invertirColores(matriz) {
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      const orig = matriz[i][j];
      resultado[i][j].r = 255 - orig.r;
      resultado[i][j].g = 255 - orig.g;
      resultado[i][j].b = 255 - orig.b;
      resultado[i][j].a = orig.a;
    }
  }
...
function convertirEscalaGrises(matriz) {
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      ...
      const v = typeof limitarValorColor === 'function' ? limitarValorColor(gris) : Math.max(0, Math.min(255, gris));
      resultado[i][j] = { r: v, g: v, b: v, a: p.a };
    }
  }
 ...
 */
  ...
  // Copia profunda fila por fila y reversa cada fila
  const resultado = matriz.map(fila => {
    const copiaFila = fila.map(p => ({ ...p }));
    copiaFila.reverse();
    return copiaFila;
  });
  return resultado;
...
function aplicarSepia(matriz) {
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      const p = matriz[i][j];
      const rNew = Math.round(0.393 * p.r + 0.769 * p.g + 0.189 * p.b);
      const gNew = Math.round(0.349 * p.r + 0.686 * p.g + 0.168 * p.b);
      const bNew = Math.round(0.272 * p.r + 0.534 * p.g + 0.131 * p.b);

      resultado[i][j].r = typeof limitarValorColor === 'function' ? limitarValorColor(rNew) : Math.max(0, Math.min(255, rNew));
      resultado[i][j].g = typeof limitarValorColor === 'function' ? limitarValorColor(gNew) : Math.max(0, Math.min(255, gNew));
      resultado[i][j].b = typeof limitarValorColor === 'function' ? limitarValorColor(bNew) : Math.max(0, Math.min(255, bNew));
      resultado[i][j].a = p.a;
    }
  }
...
function detectarBordes(matriz, umbral = 50) {
  ...
  const columnas = filas ? grises[0].length : 0;

  const resultado = Array.from({ length: filas }, () => Array.from({ length: columnas }, () => ({ r:0,g:0,b:0,a:255 })));

  for (let i = 0; i < filas; i++) {
    for (let j = 0; j < columnas; j++) {
      const centro = grises[i][j].r; // r==g==b
      const derecho = (j + 1 < columnas) ? grises[i][j + 1].r : centro;
      const abajo   = (i + 1 < filas) ? grises[i + 1][j].r : centro;

      const diff = Math.max(Math.abs(centro - derecho), Math.abs(centro - abajo));

      if (diff > umbral) {
        resultado[i][j] = { r: 255, g: 255, b: 255, a: 255 }; // borde blanco
      } else {
        resultado[i][j] = { r: 0, g: 0, b: 0, a: 255 }; // fondo negro
      }
    }
  }
